...
try:
	...
	#result, data = imap.uid('search', None, 'UNSEEN HEADER Subject "CONSEGNA: Invio a SDI - 2023" FROM "'+MAIL_SDI+'"')
    #result, data = imap.uid('search', None, 'HEADER Subject "CONSEGNA: Invio a SDI - 2023" FROM "'+MAIL_SDI+'"')
    result, data = imap.uid('search', None, 'HEADER Subject "CONSEGNA: Invio a SDI - 2023"')
			...
			if part.get('Content-Disposition') is not None:
				...
				if part.get('Content-Disposition').find('attachment') > -1:
					...
					if bool(fileName):
						...
						#folderPath = CARTELLA_VENDITE+year+'/'+month+'/';
						folderPath = CARTELLA_VENDITE+year+'/';
